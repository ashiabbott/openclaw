...
export function normalizeToolParameters(
  ...
  // Provider quirks:
  // - Google tool declarations (including google-antigravity / Cloud Code Assist)
  //   reject several JSON Schema keywords, so we scrub those.
  // - OpenAI rejects function tool schemas unless the *top-level* is `type: "object"`.
  //   (TypeBox root unions compile to `{ anyOf: [...] }` without `type`).
  // - Anthropic-native providers keep full JSON Schema draft 2020-12.
  //
